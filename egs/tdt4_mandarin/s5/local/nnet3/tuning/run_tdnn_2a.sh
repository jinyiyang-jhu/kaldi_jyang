...
affix=
common_egs_dir=
reporting_email="jyang126@jhu.edu"
# training options
...
if [ $stage -le 0 ]; then
  ...
  local/nnet3/run_ivector_common.sh --stage $ivector_stage || exit 1;
fi
...
if [ $stage -le 9 ]; then
  if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d $dir/egs/storage ]; then
    utils/create_split_dir.pl \
     /export/b0{5,6,7,8}/$USER/kaldi-data/egs/tdt4-$(date +'%m_%d_%H_%M')/s5/$dir/egs/storage $dir/egs/storage
  fi
...
if [ $stage -le 10 ]; then
  ...
  for decode_set in dev_cleanup; do
      (
...
if [ $stage -le 12 ]; then
  ...
  steps/online/nnet3/decode.sh --config conf/decode.config \
    ...
    "$graph_dir" data/dev_cleanup_hires \
