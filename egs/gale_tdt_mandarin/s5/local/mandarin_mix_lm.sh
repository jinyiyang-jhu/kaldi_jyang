...
if [ $# != 4 ]; then
  ...
  echo "E.g. $0 --ngram-order 4 --oov-sym \"<UNK>\" --prune-thres "1e-9" data/local/lm_gale data/local/lm_giga data/local/lm_mix data/dev "
  exit 1;
fi
lm_dir_1=$1
lm_dir_2=$2
...
if [ ! -f $lm_dir_mix/srilm.o${ngram_order}g.kn.gz ]; then
  ...
  ngram_opts="$lm_dir_1/srilm.o${ngram_order}g.kn.gz -weight $lambda1 -order $ngram_order\n$lm_dir_2/srilm.o${ngram_order}g.kn.gz -weight $lambda2 -order $ngram_order"
  ...
  #ngram -order $ngram_order -unk -map-unk $oov_sym -prune $prune_thres \
  #  -lm $lm_dir_1/srilm.o${ngram_order}g.kn.gz -lambda - \
  #  -mix-lm $lm_dir_2/srilm.o${ngram_order}g.kn.gz \
  #  -write-lm $lm_dir_mix/srilm.o${ngram_order}g.kn.gz
