...
cp -r $srcdir $tgtdir || exit 1;

# extra_text should not contain any utterance id
cat $extra_text | awk '{for(n=1;n<=NF;n++) print $n; }' | sort | uniq -c | \
  awk -v x=$thres '$1>x' | sort -nr > $tgtdir/extra_word_${thres}.counts || exit 1;

awk 'NR==FNR{a[$1]=$0;next}$1 in a{print a[$1]}' $extra_dict_dir/lexicon.txt $tgtdir/extra_word_${thres}.counts > $tgtdir/lexicon_extra_words.txt

awk '{for (n=1;n<=NF;n++) print $n;}' $tgtdir/lexicon_extra_words.txt | sort -u > $tgtdir/extra_word_phones.txt

cat $tgtdir/nonsilence_phones.txt $tgtdir/silence_phones.txt > $tgtdir/full_phones.txt
...
if [ `wc -l $tgtdir/extra_word_unknown_phones.txt` -ne 0 ]; then
