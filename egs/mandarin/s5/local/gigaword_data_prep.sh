...
#find $giga_dir/xin -name "*.gz" > $giga_lang_dir/giga_trans.flist || exit "Faile to find files"
...
#  local/gigaword_normalize.pl |\
#  python local/segmemt.py > $giga_lang_dir/filtered.text
cat $giga_lang_dir/filtered.text |\
  python local/segment.py > $giga_lang_dir/segmented.text
