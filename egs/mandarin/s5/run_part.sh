...
train_nj=32
decode_nj=4
extra_text=data/loca/lang_gigaword_chinese
...
if [ $stage -le 0 ]; then
#########################################################
#                       Prepare dict and LM interpolation
#########################################################
  local/prep_dict.sh \
    --extra-text data/local/lang_gigaword_chinese/segmented.text \
    data/local/dict > data/local/dict/prep_dict.log 2>&1 || exit 1;
  local/prepare_lms.sh data/local/train data/local/lang_gigaword_chinese data/local/lm || exit 1;

  utils/prepare_lang.sh data/local/dict "<UNK>" data/local/lang_with_giga data/lang_with_giga
fi

if [ $stage -le 1 ]; then
########################################################################
#                       Extract feats for GALE4_news/TDT2/TDT3
########################################################################
fi

if [ $stage -le 2 ]; then
########################################################################
#                       Segmentation for TDT2/3
########################################################################
