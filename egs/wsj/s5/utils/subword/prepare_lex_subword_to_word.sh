...
if [ $# != 4 ]; then
  echo "Usage: $0 [--oov-sym] <word-list> <subword-list> <pair-code> <subword-lang-tmp-dir>"
    echo "E.g. $0 --oov-sym=<UNK> data/lang_word/words.txt data/lang_subword/words.txt data/lang_subword/pair_code.txt data/lang_subword_to_word"
  exit 1;
fi
...
awk 'NR==FNR{a[$1]=$2;next}{str=$1;for (i=2;i<=NF;++i){str=str" "a[$i];} print str}' $subwords $tmpdir/lexicon_words_to_subwords.txt \
...
grep -w $oov_sym $words | cut -d " " -f2 > $tmpdir/oov.int
grep -w $oov_sym $words | cut -d " " -f1 > $tmpdir/oov.sym
