...
for (my $noiseIndex = 0; $noiseIndex < @noiseNames; $noiseIndex++) {
  for (my $snrIndex = 0; $snrIndex < @snrList-1; $snrIndex++) {
    ...
    my $noiseFile = "noises/postMIRS/" . $noiseName . ".raw";

    my $inList = "lists/in.$noiseName.snr$snr";
    my $outList = "lists/out.$noiseName.snr$snr";
    my $offsetList = "lists/offsets.$noiseName.snr$snr";
    ...
    # Add the noise
    system("filter_add_noise -d -m snr_4khz  -i $inList -o $outList -n $noiseFile -s $snr -a $offsetList -e $log") && die "filter_add_noise failed";
  }
}
