...
#my @noiseNames= ("destroyerops", "street", "machinegun");
my @noiseNames=("sin_100hz");
...
for (my $noiseIndex = 0; $noiseIndex < @noiseNames; $noiseIndex++) {
  for (my $snrIndex = 0; $snrIndex < @snrList; $snrIndex++) {
    ...
    my $noiseFile = "noises/preMIRS/" . $noiseName . ".wav";
    ...
    # Add the noise
    system("./add_noise.py --i $inList --o $outList --n $noiseFile --snr $snrList[$snrIndex]") && die "add_noise.py failed"
