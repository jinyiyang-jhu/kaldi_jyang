...
if [ $stage -le 1 ]; then
  ...
  if [ -z "$best_lmwt" ]; then
    ...
    cut -d " " -f1 $dir/scoring_kaldi/penalty_$best_wip/$best_lmwt.txt \
    > $dir/scoring_kaldi/penalty_$best_wip/$best_lmwt.uttid
    [ -e $dir/scoring_kaldi/wer_utt_penalty_$best_wip_lmwt_$best_lmwt.txt ] &&\
    rm $dir/scoring_kaldi/wer_utt_penalty_$best_wip_lmwt_$best_lmwt.txt
    while read -r u; do
        wer_command=`cat $dir/scoring_kaldi/wer_utt_penalty_$best_wip_lmwt_$best_lmwt_$u.txt |\
        grep "WER" | cut -d " " -f2`
        grep $u $dir/scoring_kaldi/penalty_$best_wip/$best_lmwt.txt | \
        compute-wer --text --mode=present ark:$dir/scoring_kaldi/test_filt.txt \
        ark,p:- > $dir/scoring_kaldi/wer_utt_penalty_$best_wip_lmwt_$best_lmwt_$u.txt || exit 1;
        echo $u $wer_command >> $dir/scoring_kaldi/wer_utt_penalty_$best_wip_lmwt_$best_lmwt.txt
        rm $dir/scoring_kaldi/wer_utt_penalty_$best_wip_lmwt_$best_lmwt_$u.txt
        done < $dir/scoring_kaldi/penalty_$best_wip/$best_lmwt.uttid
