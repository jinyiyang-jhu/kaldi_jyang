...
subbands_dir="/export/b07/jyang/kaldi-jyang/kaldi/egs/timit_dwt/s5/data/train_subbands"
train_dir="/export/b07/jyang/kaldi-jyang/kaldi/egs/timit_dwt/s5/data/train"
####################################################
...
for n in "${bands[@]}"
    do
        ...
        cut -d "/" -f1,21- $train_dir/wav.scp |\
        awk '{print $1" "dndir$2}' dndir=$dndir > $dndir/wav.scp
        if [ ! -s $dndir/wav.scp ];then
            echo "$dndir/wav.scp empty"
            exit 1
        fi
        cp $train_dir/{utt2spk,spk2utt,text,stm,glm} $dndir || exit "Unable to cp to $dndir"
        count=0
        total=`wc -l $train_dir/wav.scp | cut -d " " -f1`
        while read line;
        do
