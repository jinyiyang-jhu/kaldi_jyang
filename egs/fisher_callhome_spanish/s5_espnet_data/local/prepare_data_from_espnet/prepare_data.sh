...
output_dir=data
fisher_speech=data/fisher_sph.list
fisher_trans=
callhome_speech=
callhome_trans=
sph2pipe="/export/b07/jyang/kaldi-jyang/kaldi/tools/sph2pipe_v2.5/sph2pipe"
dsets=("fisher_dev" "fisher_dev" "fisher_dev2" "fisher_test" "callhome_devtest" "callhome_evltest")
src="es"
tgt="en"
...
if [ $stage -le 0 ]; then
    ...
    #    | awk '{split($0, a, "/"); split(a[9], b, "."); arr[b[1]]=$0} {split($1, c, "-"); (if c[0] in arr) print arr[c[0]]}' $fisher_speech - \
    #    | awk -F"|" '{print $1"|"$2" |"}' \
    #    > $output_dir/train/wav.scp
    cut -d "-" -f2- $espnet_train_dir/wav.scp | \
