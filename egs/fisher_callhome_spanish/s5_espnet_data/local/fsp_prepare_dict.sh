...
if [ $# -lt 2 ]; then
  echo 'Usage fsp_prepare_dict.sh data/local/data/train_all lexicon'
  exit 1;
fi
...
if [ $stage -le 3 ]; then
  ...
  sed -i "s/' //g" $tmpdir/lexicon_one_column
  cat $tmpdir/phones_extended | sort | awk '{if ($1 != "") {print;}}' > $tmpdir/phones_extended.1
  mv $tmpdir/phones $tmpdir/phones.small
  mv $tmpdir/phones_extended.1 $tmpdir/phones.tmp
  cat $tmpdir/phones.tmp | grep -v "'" | sort -o $tmpdir/phones
